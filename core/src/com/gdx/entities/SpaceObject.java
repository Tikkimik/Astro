...
public class SpaceObject {
    ...
    protected void wrap() {
        // Убираем зацикливание - объекты просто останавливаются у границ
        if(x < 0){
            x = 0;
            dx = 0; // Останавливаем движение
        }
        if(x > MyGdxGame.WIDTH * 4){ // Граница мира
            x = MyGdxGame.WIDTH * 4;
            dx = 0;
        }
        if(y < 0){
        ...
        if(y > MyGdxGame.HEIGHT * 4){ // Граница мира
            y = MyGdxGame.HEIGHT * 4;
            dy = 0;
        }
    }
}
